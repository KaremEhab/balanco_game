...
class WoodenRoutePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint ropePaint = Paint()
      ..color = const Color(0xFFD4B895) // Light sandy rope color
      ..strokeWidth = 4.0
    ...
    // Wood colors
    final Paint woodBase = Paint()..color = const Color(0xFF8B5A2B); // Rich warm wood
    final Paint woodHighlight = Paint()..color = const Color(0xFFA87545); // Top edge light
    final Paint woodShadow = Paint()..color = const Color(0xFF5A3A15); // Bottom edge dark
    final Paint nailPaint = Paint()..color = const Color(0xFF2A1F16); // Dark nails
    ...
    for (double y = 0; y < size.height; y += step) {
      ...
      canvas.drawRRect(RRect.fromRectAndRadius(plankRect, const Radius.circular(3)), woodBase);
      ...
      // Iron nails
      canvas.drawCircle(Offset(plankRect.left + 6, plankRect.center.dy), 2.0, nailPaint);
      canvas.drawCircle(Offset(plankRect.right - 6, plankRect.center.dy), 2.0, nailPaint);
    }
  }
