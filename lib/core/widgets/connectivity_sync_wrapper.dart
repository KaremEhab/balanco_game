...
class _ConnectivitySyncWrapperState extends State<ConnectivitySyncWrapper> {
  ...
  void initState() {
    ...
    if (!kIsWeb && Platform.isWindows) {
      debugPrint('Skipping onConnectivityChanged listener on Windows to prevent crashes.');
      return;
    }
  ...
  Future<void> _checkAndPromptSync() async {
    ...
    final pendingGames = await DatabaseHelper.instance.getPendingGameResults(userId);
    final pendingRuns = await DatabaseHelper.instance.getPendingInfinityRuns(userId);
