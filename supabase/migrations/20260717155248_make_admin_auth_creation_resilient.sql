-- Keep Auth user creation atomic when an administrator creates a user without
-- the profile metadata that Balanco's in-app signup form normally provides.
...
begin
  ...
  if metadata_age is not null and metadata_age ~ '^[0-9]{1,3}$' then
    ...
    -- Dashboard-created users can replace this temporary value in the
    -- existing Balanco profile editor after their first sign-in.
    requested_age := 18;
  end if;
