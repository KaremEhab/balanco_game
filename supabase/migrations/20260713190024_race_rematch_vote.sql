...
create index if not exists coop_rooms_rematch_requested_by_idx
  on public.coop_rooms(rematch_requested_by)
...
begin
  ...
  if room.mode <> 'race' or room.status <> 'ended' then
    raise exception 'The race is not ready for a rematch';
  end if;
